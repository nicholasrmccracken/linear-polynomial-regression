...
def linear_regression(X, Y):
    ...
    # Add a row of ones to X to include the bias term
    ones_row = np.ones((1, N))
    X_with_bias = np.vstack([X, ones_row])  # X becomes (D+1)-by-N

    # Compute w using (X_with_bias * X_with_bias^T)^-1 * X_with_bias * Y
    w = np.matmul(
        np.linalg.inv(
            np.matmul(X_with_bias, X_with_bias.T)
        ),
        np.matmul(X_with_bias, Y)
    ) # w is (D+1)-by-1

    # The last element of w will be the bias term
    b = w[-1, 0]
    w = w[:-1, :]  # Remove the bias term, so w is D-by-1
